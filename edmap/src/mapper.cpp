...
static void MHDaction(){
    ...
    GMap.camPitch = MHD.getCamPitch();
    GMap.camHeight = MHD.getCamHeight();
    GMap.camDistance = MHD.getCamDistance();

    printf("is3D: %u\n", (GMap.is3D())?1:0);
    bool is3D = (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(MHD.is3D)) == TRUE) ? true : false;
    if (is3D)
        GMap.setMap3D(false);
    else
        GMap.setMap2D();
...
static void MHDDestroy(){
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(MHD.is3D), (GMap.is3D()) ? TRUE : FALSE );
...
//-----------------------------------------------------
    ...
    void OnMouseMove3D(int x, int y, GdkModifierType& state, GtkWidget *control){
        ...
        POS = PointOnPlane3D(x, y, 1.0f, 500.0f, Vector3D(0,1,0) );
         printf("%u %u %d %d\n", (unsigned)(POS.x()/8.0f), (unsigned)(POS.z()/8.0f), x, y);
        char buf[255];
        sprintf(buf, "%s - Map Editor X:%d Y:%d",
                GMap.name,
                (int)(POS.x()/8.0f), (int)(POS.z()/8.0f)
...
void Drawmap3D(){
        ...
        GMap.draw3D(Pics, 0, 0, false, Collision);
        GMap.draw3D(Pics, 0, 1, false);
        glDisable(GL_LIGHTING);
        if (ShowGrid){
            glColor3f(1,1,1);
            DrawGrid3D(8.0f);
        ...
        if (Tiles3D.count()){
            ...
            GMap.draw3DTileset(frustumCenter, 300.0f, Tiles3D, Pics, 0, 2, false, Collision);
        }
    ...
    //------------------------------------
      void TTRedraw(){
       // puts("TT redraw");
        ...
        Tiles3D.draw((unsigned)gtk_adjustment_get_value((GtkAdjustment*)TT.adjpn), 0);
...
//---------------------------------
    static void LoadMap(){
        ...
        if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT){
            ...
            char * baseFileName = 0;
            baseFileName = g_file_get_basename (gf);
            baseFileName[strlen(baseFileName)-4] = '\0';

            GMap.destroy();
            GMap.load(FileName, false);
            SetupAdjustments();
            MHD.setTileset(GMap.tileset);
            MHD.setMusic(GMap.music);
            MHD.setCamPitch(GMap.camPitch);
            ...
            Pics.loadFile(buf, 0, 32, buff);
            Pics.loadFile("charai.tga", 1, 16, buff);
            sprintf(buff, "../%s/pics/%s.tga", BasePath, GMap.tileset);
            TT.loadPixbuf((gchar *)buff);
            sprintf(buff, "%s.tga", GMap.tileset);
            LoadTexForTT3D(buff);
            ...
            gdk_gl_drawable_gl_begin(GLDrawable, GLContext);
            NpcNation.destroy();
            sprintf(buf, "../%s/maps/%s.npc", BasePath, baseFileName);
...
int main( int   argc, char *argv[] ){
    ...
    NpcNation.load("../base/maps/inn.npc", "../base/", false);
    GMap.load(path, false);
    SetupAdjustments();
